...
export function Sidebar() {
  ...
  const renderLink = (item: Item) => {
    ...
    return (
      <Link key={item.path} href={item.path} onClick={() => setMobileOpen(false)}
        className={cn('flex flex-col items-center gap-[4px] py-[9px] transition-colors', active ? 'text-[#2e7de6]' : 'text-[#8fa8c8] hover:text-[#5b7fa3]')}>
        <item.Icon a={active} />
        <span className={cn('text-[9.5px] leading-none tracking-tight', active ? 'font-bold' : 'font-medium')}>{item.label}</span>
      </Link>
    );
  };
  ...
  const content = (
    <div className="flex h-full flex-col items-center">
      <nav className="flex flex-1 flex-col items-center gap-[2px] overflow-y-auto pt-2">{NAV.map(renderLink)}</nav>
      <div className="shrink-0 border-t border-[#e2e8f0] pb-2 pt-1">
